...
void FluidSim::renderImGuiSidebar() {
  ...
  // ImGui::SetNextWindowSize(, (GLsizei)io.DisplaySize.y), ImGuiCond_Always);
      ...
      ImGui::Checkbox("绘制粒子", &draw_particles_);
      
      ImGui::Separator();

      // 粒子颜色模式选择
      const char* color_modes[] = { "温度渲染", "相态渲染" };
      static int current_mode = 0;
      ImGui::Text("粒子颜色模式");
      ImGui::Combo("粒子颜色模式", &current_mode, color_modes, IM_ARRAYSIZE(color_modes));
      // 根据选择设置渲染模式
      if (current_mode == 0) {
      ...
      // 流体参数调节
      ImGui::Text("流体参数调节");
      // 假设 rho_、T_、surface_tension_ 是 FluidSim 的成员变量
      ImGui::SliderFloat("流体密度", &rho_, 0.1f, 5.0f, "%.3f");
      ImGui::SliderFloat("初始温度", &T_, 250.0f, 500.0f, "%.1f");
      ImGui::SliderFloat("表面张力", &gema, 0.0f, 20.0f, "%.2f");  // 假设表面张力变量名为 gema
...
void FluidSim::renderImGuiStatusBar() {
  // 获取主视口信息
  ImGuiViewport* viewport = ImGui::GetMainViewport();
  ImVec2 bar_size = ImVec2(viewport->Size.x, 28.0f);  // 28像素高，可根据需要调整
  ...
  // 显示模拟时间
  ImGui::SameLine();
  // ImGui::Text("模拟时间: %.2f s", outframe_ * 0.01);
  // 显示粒子数量
  ImGui::SameLine();
  ImGui::Text("粒子数量: %zu", particles_.size());
  // 显示网格大小
  ImGui::SameLine();
